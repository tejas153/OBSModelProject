...
# Begin of Comment - Tejas
#MEAN_LOS_OBS = 3
#MEAN_LOS_LDR = 12
#MEAN_LOS_PP = 48
# End of Comment - Tejas

# Unit capacities
CAPACITY_OBS = 2
CAPACITY_LDR = 6
CAPACITY_CSECT = 3    # Added Tejas
CAPACITY_PP = 24
...
# Create empty lists to get all unique Patient Types and Probabilities from file
patient_type_ls = []
pt_probability_ls = []

for each_patient in patient_data.items():
    # List of Patient Types
    patient_type_ls.append(int(each_patient[0]))

    # Probability Distribution of Patient Types
    pt_probability_ls.append(each_patient[1]['probability'])

# List of Patient Types
#patient_type_ls = [1, 2]

# Distribution of Patient Types
#pt_probability_ls = [0.8, 0.2]

# Get Number of Patient Types
pt_list_len = len(patient_type_ls)

# Initialize counter for each Patient Type
pt_counter_ls = [0 for _ in range(pt_list_len)]
...
class OBPatient(object):
    ...
    def __init__(self, arr_time, patient_id, patient_type=1, arr_stream=1,
        ...
        # Begin of Changes - Tejas
        self.route_length = len(patient_data[str(patient_type)]['sequence']) - 1

        self.planned_route_stop = [None]
        self.planned_route_stop = self.planned_route_stop + patient_data[str(patient_type)]['sequence']

        self.planned_los = [None]
        self.planned_los = self.planned_los + patient_data[str(patient_type)]['los']

        # End of Changes - Tejas
...
class OBPatientGenerator(object):
    ...
    def run(self):
        ...
        while self.env.now < self.stoptime and \
            ...
            # Generate Patient Type based on Weights
            self.patient_type = self.prng.choice(patient_type_ls, p=pt_probability_ls)
            # Get the Index number from Patient Type
            pt_index = patient_type_ls.index(self.patient_type)
            # Increment the Patient Type Counter
            pt_counter_ls[pt_index] += 1
...
csect_unit = OBunit(simenv, 'CSECT', CAPACITY_CSECT, debug=False)    # Added Tejas
...
obunits = {'OBS': obs_unit, 'LDR': ldr_unit, 'CSECT': csect_unit, 'PP': pp_unit, 'EXIT': exitflow}
...
print(csect_unit.basic_stats_msg())     # Added Tejas

# System exit stats
print("\nNum patients exiting system: {}\n".format(exitflow.num_exits))
print("Last exit at: {:.2f}\n".format(exitflow.last_exit))
#Begin Tejas
print("\nNum patients generated: {}".format(obpat_gen.num_patients_created))    # REPEATED
for i in range(len(patient_type_ls)):
    print("Patient Type {}: {}".format(patient_type_ls[i], pt_counter_ls[i]))
#End Tejas